...
public:
    ...
    void compute_BLUE()
    {
...

        {
            ...
            for (i = 0; i < min(Nx_, Ny_); i++)
                ASSERT_TRUE(analysis_matrix(i) == 2. * x(i));
            for (i = 0; i < min(Nx_, Ny_); i++)
                ASSERT_TRUE(analysis_vector(i) == 2. * x(i));
            // No observations for this part of the state.
            for (i = min(Nx_, Ny_); i < Nx_; i++)
                ASSERT_TRUE(analysis_matrix(i) == x(i));
            for (i = min(Nx_, Ny_); i < Nx_; i++)
                ASSERT_TRUE(analysis_vector(i) == x(i));
        }
        {
            StateErrorVariance B(Nx_, Nx_);
            ...
            for (i = 0; i < Nx_; i++)
                ASSERT_NEAR(analysis(i), value, 1.e-6 * value);
        }
    }
    ...
    void compute_covariance()
    {
        ...
        for (int i = 0; i < Nx_; i++)
            for (int j = 0; j < Nx_; j++)
                ASSERT_NEAR(B_sparse(i, j), B_dense(i, j),
                            1.e-6 * B_dense(i, j));
    }
...
TEST_F (BLUETest, test_compute_BLUE)
{
    int Nx[3] = {10, 10,  1};
    int Ny[3] = { 2, 10,  1};
...
TEST_F (BLUETest, test_compute_covariance)
{
    int Nx[3] = {10, 10,  1};
    int Ny[3] = { 2, 10,  1};
