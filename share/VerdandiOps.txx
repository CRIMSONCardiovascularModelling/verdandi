    ...
    */
                               ...
                               Seldon::Vector<T, PETScPar, Allocator>& value)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < default_value.GetLength(); i++)
            {
                default_data[i] = default_value.GetOnAll(i);
            }
            SetValue(name, constraint, default_data, with_default, data);
            value.Reallocate(int(data.size()));
            for (size_t i = 0; i < data.size(); i++)
            {
                value.SetBuffer(i, data[i]);
                value.Flush();
            }
        }
    }
...
//! Retrieves a value and checks if it satisfies given constraints.
               ...
               Seldon::Matrix<T, Prop, PETScMPIAIJ, Allocator>& value)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < default_value.GetM(); i++)
                for (int j = 0; j < default_value.GetN(); j++)
                {
                    default_data[index] = default_value.GetOnAll(i, j);
                    index++;
                }
            ...
            for (int i = 0; i < value.GetM(); i++)
                for (int j = 0; j < value.GetN(); j++)
                {
                    value.SetBuffer(i, j, data[index++]);
                    value.Flush();
                }
        }
    }
 ...
 //! Retrieves a value and checks if it satisfies given constraints.
    ...
    */
    ...
    ::SetValue(string name, string constraint,
               const
               vector<Seldon::Matrix<T, Prop, PETScMPIAIJ, Allocator> >&
               default_value,
               bool with_default,
               vector<Seldon::Matrix<T, Prop, PETScMPIAIJ, Allocator> >&
               value)
    {
        ...
        else if (this->Is<string>(name))
        {
            ...
            while (f.peek() != -1)
            {
                value.push_back(Seldon::Matrix<T,
                                Prop, PETScMPIAIJ, Allocator>());
                value[value.size() - 1].Read(f);
            }
            if (constraint.empty())
                return;
